...
inductive Term (bmap : Nat → Type) (p : Type) (q : Type) where
  /-- Variable (de Bruijn index) -/
  | var : Nat → Term bmap p q
  /-- Application -/
  | app : Term bmap p q → Term bmap p q → Term bmap p q
  /-- Lambda abstraction with type annotation, binds variable 0 in body -/
  | lam : Ty p q → Term bmap p q → Term bmap p q
  /-- Type abstraction (Λα:k. t), binds type variable 0 in body, gated by `p` -/
  | tyAbs : p → Kind → Term bmap p q → Term bmap p q
  /-- Type application (t [τ]), gated by `p` -/
  | tyApp : p → Term bmap p q → Ty p q → Term bmap p q
  /-- Constant at base type n -/
  | const : (i : Nat) → (c : bmap i) → Term bmap p q
  /-- Zero (natural number) -/
  | zero : Term bmap p q
  /-- Successor -/
  | succ : Term bmap p q → Term bmap p q
  /-- Primitive recursion: natrec C base step n
      C is the result type, base : C, step : Nat → C → C, n : Nat -/
  | natrec : Ty p q → Term bmap p q → Term bmap p q → Term bmap p q → Term bmap p q
